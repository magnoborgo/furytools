...
#print(cl.eVisibleFlag)
#help(nuke.curvelib.FlagType)
...
def trackerGrid(thenode):
    ...
    numTracks = 0
    inputNode.showControlPanel() #tracker4 control panel must be open or script wont work 
    ...
    for c in range(1,columns):
        ...
        for r in range(1,rows):
            ...
            y = (nodeHeight/rows)*r
            trackerNode['tracks'].setValueAt(x, nuke.frame(), numColumns*(trker+numTracks) + Trk4_parameters.index("track_x"))
            trackerNode['tracks'].setValueAt(y, nuke.frame(), numColumns*(trker+numTracks) + Trk4_parameters.index("track_y"))
            trker += 1
            print(trker+numTracks,x,y)
...
def main(thenode,furytool,mode="default"):
    ...
    if furytool == "roto":
        ...
        if inputNode.Class() != "Tracker4":
            raise Exception("Please attach a Tracker4 to TrackerNode input")
        numColumns = 31
        Trk4_parameters = ["enable","name","track_x","track_y", "offset_x","offset_y","T", "R", "S", "error","error_min", "error_max", "pattern_x", "pattern_y", "pattern_r","pattern_t",  "search_x", "search_y", "search_r", "search_t" ]
    ...
    target= nuke.toNode(thenode.name()+".Shuffle_mv") # access node inside group
    curve_tool =  nuke.toNode(thenode.name()+'.ct')
    if furytool == "roto":   
        if execution_type == 0.0: # selected 
            idxs = [point.center for shape in rotoCurves.getSelected() for point in shape if isinstance(shape, rp.Shape)]
        elif execution_type == 1.0: # all
            shapes = roto_walker(rotoRoot)
            idxs = [point.center for shape in shapes for point in shape if isinstance(shape, rp.Shape)]
            #print(idxs) 
    ...
    if len(idxs) == 0 and furytool == "roto":
        ...
        progressframes = (end_frame-start_frame)+1 if (end_frame >= start_frame) else (start_frame-end_frame)+1
        count = 1
        trackParams = {}
        if furytool == "roto":
            for point in idxs:
            ...
            for trackIdx in idxs:
                x = nodeTracks.getValue(numColumns*trackIdx + Trk4_parameters.index("track_x"))
                y = nodeTracks.getValue(numColumns*trackIdx + Trk4_parameters.index("track_y"))
                trackParams[trackIdx] = {"x_pos":x,"y_pos":y,"x":x,"y":y}
                
        
        
        frange = range(start_frame,end_frame+1) if (end_frame >= start_frame) else range(start_frame,end_frame-1,-1)
        ...
        for frame in frange:
            ...
            for trackIdx in idxs:
                ...
                u,v = sampleInRangecv(curve_tool ,trackParams[trackIdx]["x"],trackParams[trackIdx]["y"],frame)
                ...
                if (frame - start_frame) % nstep == 0 or frame == end_frame:
                    if furytool == "roto":
                        ...
                        nodeTracks.setValueAt(updatex,frame,numColumns*trackIdx + Trk4_parameters.index("track_x"))
                        nodeTracks.setValueAt(updatey,frame,numColumns*trackIdx + Trk4_parameters.index("track_y"))

                else: # remove previous keys
                    if furytool == "roto":
                        trackIdx.removePositionKey(frame)
                    elif furytool == "tracker":
                        nodeTracks.removeKeyAt(frame,numColumns*trackIdx + Trk4_parameters.index("track_x"))
                        nodeTracks.removeKeyAt(frame,numColumns*trackIdx + Trk4_parameters.index("track_y"))
